...
function SanbongTable() {
  ...
  useEffect(() => {
    const fetchFieldTypes = async () => {
      try {
        const fieldTypesResponse = await fetch('http://localhost:8080/getloaisanbong');
  ...
  const handleAddField = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/postsanbong', {
        ...
        body: JSON.stringify({
          maloaisan: fieldTypes.find(type => type.tenloaisan === selectedLoaiSan)?.maloaisan,
          tensan: tenSan,
          trangthai: trangThai,
        }),
      });
  ...
  const handleEditField = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/putsanbong/${currentFieldId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        setFields((prev) =>
          prev.map((field) =>
            field._id === currentFieldId ? { ...field, tensan: updatedField.tensan } : field
          )
        );
  ...
  const handleDeleteField = async (_id) => {
    try {
      const response = await fetch(`http://localhost:8080/deletesanbong/${_id}`, {
        method: 'POST',
      });
      if (response.ok) {
  ...
  return (
    <div className="field-table">
      ...
      <button onClick={() => setShowAddFieldTypeForm(true)}>Thêm Loại Sân</button>
      ...
      {showAddFieldForm && (
        <div className="add-form">
          ...
          <select value={selectedLoaiSan} onChange={e => setSelectedLoaiSan(e.target.value)}>
            ...
            {filteredFields.map((field, index) => (
              <tr key={field._id}>
                ...
                <button onClick={() => openEditDialog(field)}>Sửa</button>
                  ...
                  <button onClick={() => handleDeleteField(field._id)}>Xóa</button>
                </td>
              </tr>
            ))}
          </tbody>

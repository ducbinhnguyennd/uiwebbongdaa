...
const ThanhToanNhanh = () => {
  ...
  const handleServiceSelect = async (service) => {
    ...
    try {
      const endpoint = service.type === 'drink' 
      ...
      await fetchInvoices();
      const updatedInvoice = invoices.find(invoice => invoice.idhoadon === selectedInvoice.idhoadon);
  ...
  const handleRemoveService = async (serviceId) => {
    ...
    try {
      const isDrink = selectedInvoice.douong.some((service) => service._id === serviceId);
      const isRental = selectedInvoice.dothue.some((service) => service._id === serviceId);
  
      if (isDrink) {
        await axios.post(`http://localhost:8080/xoadouonghoadon/${serviceId}/${selectedInvoice.idhoadon}`);
        alert("Đồ uống đã được xóa khỏi hóa đơn!");
      } else if (isRental) {
        await axios.post(`http://localhost:8080/xoadothuehoadon/${serviceId}/${selectedInvoice.idhoadon}`);
  ...
  const calculateTotalServicePrice = () => {
    ...
    const totalDouong = selectedInvoice.douong.reduce((total, item) => total + item.thanhtien, 0);
    const totalDothue = selectedInvoice.dothue.reduce((total, item) => total + item.thanhtien, 0);
  ...
  const handlePayment = async () => {
    ...
    const requestBody = {
      phuphi: phuPhi,
      method,
      ...(method === 'tiền mặt' ? { tienkhachtra: tienKhachTra, tienthua: tienThua } : {}),
      ...(method === 'chuyển khoản' ? { sotaikhoan: soTaiKhoan, nganhang: nganHang } : {}),
    };

    try {
      await axios.post(`http://localhost:8080/posthoadon/${selectedInvoice.idhoadon}`, requestBody);
  ...
  return (
    <div className="thanh-toan-nhanh-container">
      ...
      <div className="content">
          ...
          <table className="invoice-table">
            ...
            <tbody>
              {invoices.map((invoice, index) => (
                <tr key={invoice.idhoadon} onClick={() => handleInvoiceSelect(invoice)}>
                  <td>{index + 1}</td>
        ...
        <div className="used-services">
          ...
          <div className="services-list">
            {selectedInvoice && (
              <>
                {selectedInvoice.douong.concat(selectedInvoice.dothue).map((service) => (
                  <div key={service._id} className="service-item">
                    <img className='service-image' src={service.image} alt={service.type === 'drink' ? service.tendouong : service.tendothue} />
                    <h4>{service.type === 'drink' ? service.tendouong : service.tendothue}</h4>
                    <p>Số lượng đã chọn: {service.soluong}</p>
                    <button onClick={() => handleRemoveService(service._id)}>Xóa</button>
                  </div>
                ))}
              </>
            )}
      ...
      <div className="summary-item">
        ...
        <h4>{selectedInvoice?.booking?.hovaten} SĐT {selectedInvoice?.booking?.phone}</h4>
      </div>
      
      <div className="summary-item">
        <label>{selectedInvoice?.booking?.loaisanbong} - {selectedInvoice?.booking?.ca}</label>
      </div>
      
      <div className="summary-item">
        <label>Tiền Sân</label>
        <input type="number" value={selectedInvoice?.booking?.giaca || 0} readOnly />
      </div>
      ...
      {method === 'tiền mặt' && (
        <>
          <div className="summary-item">
            ...
            <input type="number" value={tienKhachTra} onChange={(e) => setTienKhachTra(e.target.value)} />
          </div>
          
          <div className="summary-item">
            <label>Tiền Thừa</label>
            <input type="number" value={tienKhachTra - (selectedInvoice?.tongtien + phuPhi)} readOnly />
          </div>
        </>
      )}
        ...
        {isModalOpen && (
          <div className="modal">
            <div className="modal-content">
              <h3>Chọn Dịch Vụ</h3>
              <button className="close-modal" onClick={() => setIsModalOpen(false)}>Đóng</button>
              <div className="services-grid">
                {services.map((service) => (
                  <div key={service._id} className="modal-service-item">
                    <img className='modal-service-image' src={service.image} alt={service.type === 'drink' ? service.tendouong : service.tendothue} />
                    <h4>{service.type === 'drink' ? service.tendouong : service.tendothue}</h4>
                    ...
                    />
                    <button onClick={() => handleServiceSelect(service, service.soluong)}>Thêm</button>
                  </div>
                ))}
              </div>
            </div>
